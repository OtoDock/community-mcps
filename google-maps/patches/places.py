...
class PlacesTool(BaseTool):
    ...
    async def execute(self, arguments: dict[str, Any]) -> dict[str, Any]:
        """Execute places search using Text Search API (supports keywords)."""
        try:
            ...
            # Execute API call using Text Search API (supports keyword queries)
            loop = asyncio.get_event_loop()
            result = await loop.run_in_executor(
                None,
                lambda: self._search_text_api(lat, lng, radius, keyword, place_type),
            )
    ...
    def _search_text_api(
        ...
        """Search places using Text Search API — supports keyword queries natively."""
        ...
        # Build text query — combine keyword with type if both provided
        text_query = keyword
        if place_type and place_type.lower() not in keyword.lower():
            text_query = f"{keyword} {place_type}"

        # Build the request with location bias (circle around coordinates)
        request = places_v1.SearchTextRequest(
            ...
            rank_preference=places_v1.SearchTextRequest.RankPreference.DISTANCE,
        )

        # Optionally restrict to a specific type
        if place_type:
            request.included_type = place_type

        # Field mask required by the API
