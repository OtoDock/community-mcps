...
_CLIP_SCHEMA_DOC = (
    ...
    "(seconds; per-sample afade — use these for 20–50 ms splice edges), "
    "gain_keyframes: [{t, gain_db}, …] = GAIN AUTOMATION (clip-local "
    "seconds, absolute dB at each point, linear between points, held "
    "outside; replaces gain_db — one music clip can dip and return without "
    "being split), duck: true or {threshold, ratio, attack, release} — "
    "ducks this clip under everything that is not ducked: base-track "
    "audio plus other audio clips (a music bed dips under a voice-over "
    "clip even when the base video is silent). Base-track clips take the "
    "same gain_keyframes plus audio_fade_in/audio_fade_out (sound only; "
    "picture fades are transitions).\n"
    "AUDIO SWEETENING (base + audio media clips): audio: {denoise: true "
    "(broadband) | \"voice\" (speech NN model) | 0–1 (the voice model at "
    "that wet mix — blend when a voice comes through room speakers, a flat "
    "denoise mutes it) | {strength: dB, floor_db} | {mode: \"voice\", mix}, "
    "eq: {preset: voice|music|bright|warm|telephone} or {bands: [{f, "
    "gain_db, q?|width_hz?}]}, compress: true | {threshold_db, ratio, "
    "attack, release, makeup_db}, deess: true | {intensity}, level: true | "
    "{window: s, max_gain_db, target_db} = LEVELLER for uneven speech "
    "(dynaudnorm; levels drift slower than the window — 4.2 s default, "
    "window 1.5–2 for phrase-to-phrase swings)} — fixed order "
    "denoise→eq→compress→deess→level, and the chain is latency-neutral "
    "(the denoisers' 10/25 ms processing delay is compensated inside it). "
    "Master bus: audio_master adds eq/compress/limiter (limiter: true | "
    "{ceiling_db} — the true-peak safety when loudnorm is off) and "
    "loudnorm: true | {target_lufs, true_peak, lra, mode: auto|linear|"
    "dynamic}. auto (default) is ffmpeg's two-pass loudnorm, which "
    "silently switches to a DYNAMIC program compressor when a static gain "
    "would breach true_peak or the mix's LRA exceeds lra (the final "
    "render reports normalization_type and warns); linear = one static "
    "gain to the target + a true-peak limiter at the ceiling, LRA "
    "untouched — the mastering choice for music and mixed programmes; "
    "dynamic = the leveller on purpose.\n"
...
TOOLS = [
    Tool(
        ...
        description=(
            ...
            "resolution/fps, a rotation tag when the file carries one (the "
            "size players show; footage stored turned WITHOUT a tag needs "
            "edit_video rotate), the colour tags (matrix / primaries / "
            "transfer with an HLG|PQ|SDR verdict / range), whether the MP4/MOV "
    ...
    Tool(
        ...
        description=(
            ...
            "source pixels). Tracks frame-by-frame WITHIN ONE SHOT — a cut "
            "in the span loses the target and the result says where. The "
            "result also WARNS when the box does not look like a distinct "
            "subject (a box on open water or sky 'tracks' fine while "
            "drifting) — re-check the coordinates when that appears. "
            "smoothing 0-1 (0.5 default; higher = heavier cinematic "
            "damping), keyframe_interval seconds (0.25 default). Related: "
            "edit_video smart_reframe {aspect, track_box, smoothness?} = "
            "subject-FOLLOWING vertical crop for the whole clip (faceless "
            "subjects: boats, cars, products); edit_video blur_region = "
            "tracked blur patch."
        ),
        inputSchema={
            "type": "object",
    ...
    Tool(
        ...
        description=(
            ...
            "offset, a confidence, the implied placement (where the target "
            "starts on the ref, how long the files overlap, whether their "
            "loudness envelopes agree there), the runner-up candidates for "
            "a timecode cross-check, and the two ways to apply it in a "
            "composition (target in = ref.in + offset with the same start, "
            "or target start = ref.start − offset). A placement that cannot "
            "hold is marked IMPLAUSIBLE. max_offset = search window in "
            "seconds (default 60; the searched range is bounded by the "
            "files' own lengths and printed). Both files need audio; works "
    ...
    Tool(
        ...
        description=(
            ...
            "uppercase, font_size, max_words_per_cue, offset, font: a "
            "family shipped in the image (Inter, Roboto, Comfortaa, "
            "Montserrat, Liberation Sans, DejaVu Sans, Noto Sans, JetBrains "
            "Mono) or font_file: a workspace .ttf/.otf (its declared family "
            "is used), bold (default true), italic} · "
            "set_audio_master {gain_db, loudnorm: true|{target_lufs, "
            "true_peak, lra, mode: auto|linear|dynamic}, eq, compress, "
            "limiter}.\n"
    ...
    Tool(
        ...
        description=(
            ...
            "two-pass EBU R128 loudness normalization — the deliverable; the "
            "result reports which normalization ran (linear gain vs the "
            "dynamic leveller — auto mode warns when ffmpeg fell back to "
            "dynamic; set audio_master.loudnorm.mode 'linear' to forbid it) "
            "and the loudness/true-peak/LRA MEASURED on the encoded file "
            "(AAC adds up to ~1 dB of true-peak overshoot above the PCM "
            "ceiling — set true_peak 1 dB lower for a strict delivered "
            "ceiling). "
    ...
    Tool(
        ...
        description=(
            ...
            "Comfortaa, Montserrat, Noto, color emoji, JetBrains Mono) are "
            "available by name; for transparent output leave the page "
            "background unset."
        ),
        inputSchema={
            "type": "object",
    ...
    Tool(
        ...
        description=(
            ...
            "rotate {degrees: 90|180|270|-90, flip?: 'h'|'v'} (bakes a "
            "quarter turn into the pixels — footage a rig stores turned "
            "with an identity matrix; a source's rotation tag is applied "
            "first and the output carries none; HDR tags survive) · "
            "extract_audio {format: wav|mp3|aac|flac} · replace_audio "
            "{audio_path, mix_original_db?} · enhance_audio {preset: "
            "voice|music, denoise?: 0–1, level?: true} (voice: NN denoise + "
            "clarity EQ + compression + de-ess + limiter — the one-shot "
            "interview/VO cleanup; stages can be overridden, e.g. denoise: "
            "false, or denoise: 0.4 to keep a voice that comes through room "
            "speakers; level: true adds the leveller for phrase-to-phrase "
            "swings; output is 48 kHz and latency-neutral — the chain's "
            "processing delay is compensated, nothing to re-align) · "
            ...
            "mute · volume {db} · "
            "loudness_normalize {target_lufs: -14, true_peak: -1.5, lra: 11, "
            "mode: auto|linear|dynamic} (two-pass R128; auto is ffmpeg's "
            "choice and WARNS when it fell back to the dynamic leveller, "
            "linear = static gain + true-peak limiter, LRA kept; the note "
            "carries the loudness measured on the encoded file) · "
            "burn_subtitles {subtitle_path: .transcript.json/.srt/.ass, "
            "preset: karaoke|word-pop|clean|minimal, position, "
            "highlight_color, uppercase, font, font_file, bold, italic} · "
            "to_gif {fps, width, start, end} · "
