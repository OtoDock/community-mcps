...
# does) and periodic `ping` — but then FAIL TO CORRELATE the client's POSTed
# response back to the pending request. The server therefore blocks every first
# tool call until its 60s server-request timeout fires, so a `browser_navigate`
# "hangs ~60s and then times out with a blank page" while a follow-up screenshot
# (after the stuck request has already timed out) works.
#
# Confirmed on **@playwright/mcp@0.0.55** (the version camoufox pins for patched-
# Firefox connect compatibility). Reproduced driving the MCP DIRECTLY (no sidecar,
# no satellite tunnel), answering `roots/list`+`ping` correctly — still 60.6s. So
# it is the MCP build, not OtoDock's sidecar/tunnel. @playwright/mcp@0.0.68 fixes
# it (navigate 0.06s) but is NOT camoufox-compatible: it is hard-pinned to an npm
# playwright *alpha* build that cannot be matched to camoufox 0.4.11's PyPI
# playwright (PyPI ships only the 1.59.0 *release* → connect `428`; forcing the
# release breaks 0.0.68, which imports alpha-only internals). See
# proxy/docs/mcps/DOCKER-MCP-STREAMING.md ("@playwright/mcp@0.0.55 server-request
# correlation workaround").
#
# When ``OTO_MCP_SUPPRESS_SERVER_REQUESTS=1`` the sidecar makes the server unable
# to issue those requests at all (so they can never block a tool call):
#   (a) strips ``capabilities.roots`` from the client's ``initialize`` request →
#       the server never sends ``roots/list``; and
#   (b) rejects the OPTIONAL standalone server→client SSE ``GET`` with ``405`` →
#       no channel for ``ping``; a spec-compliant client then receives results on
#       its POST responses (verified: `browser_navigate` returns in ~5s, not 60s).
# A browser MCP needs neither filesystem `roots` nor a standalone server stream.
#
# TO REMOVE: when camoufox runs an @playwright/mcp (+ a matching playwright) build
# that correlates server→client responses correctly, unset
# OTO_MCP_SUPPRESS_SERVER_REQUESTS in that MCP's compose env and confirm a
# `browser_navigate` returns sub-second. Then this whole block + the two gated
# call sites in `proxy_handler` can be deleted — they are self-contained.
# ---------------------------------------------------------------------------
SUPPRESS_SERVER_REQUESTS = os.environ.get("OTO_MCP_SUPPRESS_SERVER_REQUESTS", "0") == "1"


def _strip_roots_capability(body: bytes) -> bytes:
    """Remove ``capabilities.roots`` from an MCP ``initialize`` request body so the
    server never issues a ``roots/list`` server→client request. Best-effort: any
    non-initialize / unparseable body is returned untouched. Part of the
    ``OTO_MCP_SUPPRESS_SERVER_REQUESTS`` workaround above."""
    try:
        msg = json.loads(body)
    except Exception:
        return body
    if not isinstance(msg, dict) or msg.get("method") != "initialize":
        return body
    caps = (msg.get("params") or {}).get("capabilities")
    if isinstance(caps, dict) and caps.pop("roots", None) is not None:
        try:
...
async def proxy_handler(request):
    ...
    # Workaround (OTO_MCP_SUPPRESS_SERVER_REQUESTS — see top of file): reject the
    # OPTIONAL standalone server→client SSE GET so a buggy MCP can't open a channel
    # for server-initiated requests (ping) that would block tool calls. 405 is the
    # spec-defined "no SSE stream on GET"; a compliant client falls back to
    # receiving responses on its POST requests.
    if SUPPRESS_SERVER_REQUESTS and request.method == "GET":
        return web.Response(status=405, text="standalone SSE GET disabled")
    ...
    # Workaround (OTO_MCP_SUPPRESS_SERVER_REQUESTS — see top of file): strip the
    # client's `roots` capability from `initialize` so the buggy MCP never issues a
    # `roots/list` server→client request. `content-length` is hop-by-hop (dropped
    # above) so the rewritten body's length is recomputed downstream.
    if SUPPRESS_SERVER_REQUESTS and body:
        body = _strip_roots_capability(body)
