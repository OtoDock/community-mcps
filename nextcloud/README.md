...
| Credentials | **Per-user** (`NEXTCLOUD_USERNAME`, `NEXTCLOUD_PASSWORD`) |
...
- Admin sets `NEXTCLOUD_URL` once on the MCP's admin page.
- Each user supplies their own `NEXTCLOUD_USERNAME` + `NEXTCLOUD_PASSWORD` (or App Password) in their per-user MCP settings.
...
- `package.json` — pins the upstream npm version. `node_modules/` is not committed.
